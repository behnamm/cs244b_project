...
// This macro forces the compiler to align allocated memory to the cash line size.
// This is necessary as a performance boost for the data portion of packet
// buffers.
...
class SolarFlareDriver : public Driver {
  public:
    ...
    // This is the size, in bytes, of the packet buffer for both RX and TX ring.
     ...
     */
     struct PacketBuff {
        ...
        // will return it to us in events. It helps to easily identify this packet
        // buffer among all the PacketBuff registered to the NIC.
        int id;
    ...
    class RegisteredBuffs {
      ...
      PRIVATE:
        ...
        // the NIC for DMA access and has been subdivided into packet buffers in
        // this container.
        char* memoryChunk;
  ...
  PRIVATE:
    ...
    // The address to the beginning of RAMCloud's logMemory that is registered to
    // SolarFlare NIC. Along with the logBytes defines the region of logMemory
    // that SolarFlare NIC has DMA access to it and cad do zero copy transmission
    // of packets from that region.
    uintptr_t logBase;
    ...
    // This file descriptor is only necessary when this driver is being used on a
    // client side and no locator string has been provided.
    int fd;
    
    // socket address for the file descriptor above
    sockaddr sockAddr;
